...
if SINR_vs_rcell==1
udn.UE_num=64;               % number of UEs in a sector
rcell=[20:10:300];           % cell radius, m 
udn.accuracy=3;              % UE location uncertainty diameter, m
...
figure(21);
plot(rcell,SINR_S_est_mean,'r-','linewidth',1); hold on;
plot(rcell,SINR_S_tru_mean,'r--','linewidth',1); hold on;
plot(rcell,SINR_SC_est_mean,'b-','linewidth',1); hold on;
plot(rcell,SINR_SC_tru_mean,'b--','linewidth',1); hold on;
plot(rcell,SINR_SCN_est_mean,'g-','linewidth',1); hold on;
plot(rcell,SINR_SCN_tru_mean,'g--','linewidth',1); hold on;
grid on; axis('tight'); ylabel('SINR, dB'); xlabel('R, m');
legend('UE_{est} S','UE_{tru} S','UE_{est} S+C','UE_{tru} S+C','UE_{est} S+C+N','UE_{tru} S+C+N'); 
figure(22);
plot(rcell,az_3dB_mean,'m-','linewidth',2); hold on;
plot(rcell,el_3dB_mean,'g--','linewidth',2); grid on;
axis('tight'); ylabel('HPBW, \circ'); xlabel('R, m');
legend('\phi_{3dB}','\theta_{3dB}'); 
end

%% SINR_vs_UE_num
if SINR_vs_UE_num==1
UE_num=[1:1:64];             % number of UEs in a sector
udn.rcell=100;               % cell radius, m 
udn.accuracy=3;              % UE location uncertainty diameter, m
for i=1:length(UE_num)
    UE_num(i)
    udn.UE_num=UE_num(i);
    % hexagonal scenario of territorial distribution of gNB
    [gNB, gNB_cell, gNB_sector]=lab_grid(udn.rcell);
...
figure(31);
plot(UE_num,SINR_S_est_mean,'r-','linewidth',1); hold on;
plot(UE_num,SINR_S_tru_mean,'r--','linewidth',1); hold on;
plot(UE_num,SINR_SC_est_mean,'b-','linewidth',1); hold on;
plot(UE_num,SINR_SC_tru_mean,'b--','linewidth',1); hold on;
plot(UE_num,SINR_SCN_est_mean,'g-','linewidth',1); hold on;
plot(UE_num,SINR_SCN_tru_mean,'g--','linewidth',1); hold on;
grid on; axis('tight'); ylabel('SINR, dB'); xlabel('K UEs');
legend('UE_{est} S','UE_{tru} S','UE_{est} S+C','UE_{tru} S+C','UE_{est} S+C+N','UE_{tru} S+C+N'); 
figure(32);
plot(UE_num,az_3dB_mean,'m-','linewidth',2); hold on;
plot(UE_num,el_3dB_mean,'g--','linewidth',2); grid on;
axis('tight'); ylabel('HPBW, \circ'); xlabel('K UEs');
legend('\phi_{3dB}','\theta_{3dB}'); 
end